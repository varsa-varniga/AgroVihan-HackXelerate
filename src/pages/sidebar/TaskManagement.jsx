...
import { 
  ...
  DialogContent, DialogTitle, Paper, Grid, MenuItem, 
  Select, FormControl, InputLabel, Chip, IconButton,
  Checkbox, CircularProgress, Alert, Divider, Card, 
  CardContent, Tabs, Tab, LinearProgress, ThemeProvider,
  createTheme, alpha
} from '@mui/material';
import { 
...
export default function TaskManagementSystem({ userId }) {
  ...
  const categories = ['Maintenance', 'Planting', 'Harvesting', 'Treatment', 'Fertilization', 'Irrigation', 'Other'];
  const priorities = ['Low', 'Medium', 'High', 'Urgent'];
  
  // Reference to the tasks collection
  const tasksCollectionRef = collection(db, 'users', userId || 'anonymous', 'tasks');
  ...
  const getPriorityColor = (priority) => {
    switch (priority) {
      case 'Low': return { color: 'info', bgcolor: theme.palette.info.light };
      case 'Medium': return { color: 'success', bgcolor: theme.palette.success.light };
      case 'High': return { color: 'warning', bgcolor: theme.palette.warning.light };
      case 'Urgent': return { color: 'error', bgcolor: theme.palette.error.light };
  ...
  const filteredTasks = tasks.filter(task => {
    ...
    const statusMatch = 
      ...
      filter === 'upcoming' ? !task.completed :
      filter === 'overdue' ? (!task.completed && isOverdue(task.dueDate)) : true;
      ...
      >
            ...
            {sortedTasks.length === 0 ? (
              ...
              sortedTasks.map(task => (
                <Card 
                  ...
                  sx={{ 
                    mb: 2, 
                    bgcolor: task.completed ? alpha(theme.palette.success.light, 0.5) : 'white',
                    borderRadius: 2,
                    borderLeft: 4,
                    borderColor: `${theme.palette[getPriorityColor(task.priority).color].main}`,
                    transition: 'all 0.2s ease',
                    boxShadow: task.completed ? 'none' : `0 2px 8px ${alpha(theme.palette.common.black, 0.05)}`,
                    '&:hover': {
                      boxShadow: task.completed ? 'none' : `0 4px 12px ${alpha(theme.palette.common.black, 0.08)}`,
                        ...
                        <Box>
                          ...
                          <Box display="flex" flexWrap="wrap" gap={1} mt={1}>
                            ...
                            <Chip 
                              ...
                              sx={{ 
                                borderRadius: 1,
                                bgcolor: getPriorityColor(task.priority).bgcolor,
                                color: theme.palette[getPriorityColor(task.priority).color].dark,
                                fontWeight: 500
                              }}
                            />
                          </Box>
        ...
        >
          ...
          <Box display="flex" flexDirection="column" alignItems="center" p={1}>
            ...
            <Typography variant="h6" color={theme.palette.success.main} fontWeight={600}>
          ...
          <Box display="flex" flexDirection="column" alignItems="center" p={1}>
            ...
            <Typography variant="h6" color={theme.palette.info.main} fontWeight={600}>
          ...
          <Box display="flex" flexDirection="column" alignItems="center" p={1}>
            ...
            <Typography variant="h6" color={theme.palette.error.main} fontWeight={600}>
              {tasks.filter(t => !t.completed && new Date(t.dueDate) < new Date()).length}
            </Typography>
          </Box>
        </Box>
        ...
        <Dialog 
          ...
          PaperProps={{
            sx: {
              borderRadius: 2,
              backgroundImage: `linear-gradient(to bottom, ${alpha(theme.palette.primary.light, 0.05)}, white)`
            }
          }}
        >
          <DialogTitle sx={{ 
            ...
            <Grid container spacing={3} sx={{ mt: 0 }}>
              <Grid item xs={12}>
                <TextField
                  ...
                  helperText={!newTask.title && isDialogOpen ? 'Title is required' : ''}
              ...
              <Grid item xs={12} md={6}>
                <TextField
                  ...
                  helperText={!newTask.dueDate && isDialogOpen ? 'Due date is required' : ''}
                  sx={{
                    '& .MuiOutlinedInput-root': {
                      '&.Mui-focused fieldset': {
                        borderColor: theme.palette.primary.main,
                  ...
                  <Select
                    ...
                    sx={{
                      '& .MuiOutlinedInput-notchedOutline': {
                        borderColor: alpha(theme.palette[getPriorityColor(newTask.priority).color].main, 0.5),
                      },
                      '&.Mui-focused .MuiOutlinedInput-notchedOutline': {
                        borderColor: theme.palette[getPriorityColor(newTask.priority).color].main,
                      },
                    }}
                  >
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center' }}>
                          <Box 
                            sx={{ 
                              ...
                              height: 12, 
                              borderRadius: '50%', 
                              bgcolor: theme.palette[getPriorityColor(priority).color].main,
                              mr: 1
                            }} 
                          />
                          {priority}
