...
export default function CropMonitoringDashboard() {
  ...
  const FIREBASE_URL =
    "https://agrovihan-default-rtdb.asia-southeast1.firebasedatabase.app/dht.json";
  ...
  const fetchSensorData = async () => {
    ...
    try {
      ...
      setSensorError(
        "Failed to fetch sensor data. Please check your connection."
      );
  ...
  const fetchSuggestion = async () => {
    if (!cropType || !cropPhase || !weather) {
      setSuggestionError(
        "Please select crop type, crop phase, and wait for weather data."
      );
      return;
    }
    ...
    try {
      ...
      setSuggestionError(
        "Failed to fetch suggestion. Showing mock suggestion."
      );
      setSuggestion(
        "Based on current weather, reduce irrigation and monitor for diseases due to humidity."
      );
    } finally {
  ...
  const renderWeatherSummary = () => {
    if (loading)
      return (
        <LinearProgress
          sx={{ my: 4, backgroundColor: themeColors.secondary }}
          color="success"
        />
      );
    if (error)
      ...
      >
        ...
        >
          <CardContent sx={{ textAlign: "center" }}>
            <ThermostatIcon
              sx={{ color: themeColors.primary, fontSize: 32, mb: 1 }}
            />
        ...
        >
          <CardContent sx={{ textAlign: "center" }}>
            ...
            <Typography
              ...
              fontWeight="bold"
            >
              {weather.rain ? (weather.rain["1h"] || 0).toFixed(1) : "0"} mm
            </Typography>
          </CardContent>
        </Card>
      </Box>
  ...
  const renderSensorData = () => {
    if (loadingSensorData) {
      return (
        <LinearProgress
          sx={{ my: 2, backgroundColor: themeColors.secondary }}
          color="success"
        />
      );
    }
    ...
    >
      <Container maxWidth="md">
        ...
        >
          ...
          >
            <CropIcon
              sx={{ color: themeColors.primary, fontSize: 36, mr: 1 }}
            />
            <Typography
              variant="h4"
              fontWeight="bold"
          ...
          >
            <Stack direction="row" spacing={1} alignItems="center">
              ...
              <Typography
                variant="h6"
                sx={{ color: themeColors.textPrimary, fontWeight: 600 }}
              >
